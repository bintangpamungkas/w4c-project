...
<section>
	<div id="<?= $section_slug ?>" class="dzsparallaxer auto-init height-is-based-on-content use-loading mode-scroll loaded dzsprx-readyall "
		...
		<div class="divimage dzsparallaxer--target w-100 g-bg-pos-bottom-center"
		     style="height: 100%;background: url(<?= base_url(DIR_IMG . 'service/' . $service_id . '/bg/' . $section->section_image) ?>);background-size: cover;<?= $this->agent->is_mobile() ? 'background-position-x: right;' : '' ?>"></div>
					...
					<div class="col-md-6 col-10">
						<?php if($service->service_name == "Peralatan & Perlengkapan Mengompos") : ?>
							<h3 class="g-color-black-opacity-0_8 g-font-weight-700 <?= $this->agent->is_mobile() ? 'g-font-size-35 text-center' : 'g-font-size-50' ?> mb-4 g-font-asap"
								style="line-height: 1.2;width:700px"><?= $service->service_name ?></h3>
						<?php else : ?>
							<h3 class="g-color-black-opacity-0_8 g-font-weight-700 <?= $this->agent->is_mobile() ? 'g-font-size-35 text-center' : 'g-font-size-50' ?> mb-4 g-font-asap"
						...
						<div class="row">
							<div class="col-md-12"	>
								<h3
									class="g-color-black-opacity-0_7 <?= $this->agent->is_mobile() ? 'g-font-size-14  text-center' : 'g-font-size-16' ?> g-line-height-1_5 mb-3"><?= ucfirst($section->section_message) ?></h3>
							</div>
						</div>
