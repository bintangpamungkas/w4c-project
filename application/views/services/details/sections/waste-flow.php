...
<section class="g-bg-white <?= $this->agent->is_mobile() ? 'g-pt-80 g-pb-50' : 'g-py-140' ?>">
	...
	<div class="container">
		<div class="text-center g-mb-50">
			...
			<?= $service->service_id == 1 ? '<div class="mt-2 mb-4"> ' . get_lang('residual-waste-treatment-at-the-landfill') . ' </div>' : '' ?>
				...
				<div class="row no-gutters mt-4">
					...
					foreach ($flow_category as $key => $category) {
						...
						<div class="col text-center g-px-0">
							<div class="my_tab_2 g-cursor-pointer nav-link g-font-weight-700 g-font-size-<?= $this->agent->is_mobile() ? '14 g-py-10 g-px-5 ' : '15 g-py-12  g-px-20' ?> <?= $key == 0 ? 'tab-active' : '' ?>" data-tab="#<?= $category ?>" style="<?= $this->agent->is_mobile() ? 'min-height: 40px' : 'min-height: 50px' ?> ; color:#aaa">
								<?= get_lang($category) ?>
							</div>
						</div>
						<?php
