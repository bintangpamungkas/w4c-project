  ...
  <?= ($this->agent->is_mobile()) ? '<div class="js-scrollbar card-block  u-info-v1-1 g-bg-white-gradient-v1--after g-pa-0 full-height"> ': '' ?>
        ...
        <?php if ($this->agent->is_mobile()): ?>
          <?php if (empty($nav['menu'])): ?>
            <li class="nav-item  g-mx-10--lg g-mx-15--xl">
              <a id="nav-link-pages" class="nav-link g-py-7 g-px-0 text-uppercase" href="<?=$nav['url']?>" > <?=$nav['title']?></a>
            </li>
          <?php else: ?>
            <li class="hs-has-sub-menu nav-item  g-mx-10--lg g-mx-15--xl" data-animation-in="fadeIn" data-animation-out="fadeOut">
              <a id="nav-link-pages" class="nav-link g-py-7 g-px-0 text-uppercase" href="#" aria-haspopup="true" aria-expanded="false" aria-controls="nav-submenu-pages"><?=$nav['title']?></a>
              <ul class="mega-menu hs-sub-menu list-unstyled u-shadow-v11 g-brd-0 g-min-width-220 g-mt-18 g-mt-8--lg--scrolling" id="nav-submenu-pages" aria-labelledby="nav-link-pages">
                <!-- Pages - Others -->
                <?php foreach ($nav['menu'] as $menu): ?>
                  <?php if (empty($menu->menu)): //don't have menu?>
                    <li class="dropdown-item">
                      <a id="nav-link--pages--others" class="nav-link g-color-black" href="<?=$menu->url?>"> <?=$menu->title?></a>
                    </li>
                  <?php else: //has menu?>
                    <li class="dropdown-item hs-has-sub-menu ">
                      <a id="nav-link--pages--others" class="nav-link g-color-black" href="#" aria-haspopup="true" aria-expanded="false" aria-controls="nav-submenu--pages--others"><i class="<?=$menu->icon?> g-mr-10"></i><?=$menu->title?></a>

                      <ul class="hs-sub-menu list-unstyled u-shadow-v11 g-brd-0 g-min-width-220 g-mt-minus-2" id="nav-submenu--pages--others" aria-labelledby="nav-link--pages--others">
                        <?php foreach ($menu->menu as $submenu): ?>
                          <?php if (empty($submenu->menu)): ?>
                            ...
                            <?php else: ?>
                              <li class="dropdown-item hs-has-sub-menu ">
                                <a class="nav-link g-color-black" href="#" aria-haspopup="true" aria-expanded="false" aria-controls="nav-submenu--pages--404"><?=$submenu->title?></a>
    ...
    if (!$this->agent->is_mobile()){
      ...
      <div class="hs-has-sub-menu nav-item g-mx-20--lg" data-animation-in="fadeIn" data-animation-out="fadeOut" data-position="right">
        <a href="javascript:void()" class="nav-change-scroll g-color-black g-font-size-12 nav-link px-0" style="line-height: 1" aria-haspopup="true" aria-expanded="false">
