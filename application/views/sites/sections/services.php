<section class="g-bg-secondary">
	...
	<div class="container <?= $this->agent->is_mobile() ? 'g-pt-150 g-pb-25' : 'g-py-70' ?>">
						...
						<div class="autocomplete">
							<div class="input-group g-brd-primary--focus pull-right">
								<div class="input-group-prepend">
									<span class="input-group-text rounded-0 g-bg-white g-color-gray-light-v1 g-pa-10 border-right-0 g-box-shadow-none" style="border:1px solid #0B90B9;"><i class="fa fa-map-marker"></i></span>
								</div>
								<input id="input_city"
								       class="form-control form-control-md border-left-0 rounded-0 g-pa-10 pl-0 border-right-0"
								       type="text"
								       name="city"
								       value="<?= $input_city ?>"
		...
		<div class="">
							...
							<div id="service-<?= $target->service_target_id ?>-carousel" class="owl-carousel row owl-theme" style="margin: 0!important;">
								...
								foreach ($target->list as $service) {
									if ($service->service_id != $service_id) {
										?>
										<div class="service-list-<?=$target->service_target_id?> g-mb-20 g-bg-white g-mt-10 box-shadow-down">
			...
			?>
			<div class="service-all-<?=$target->service_target_id?> g-mx-30">
				<div class="g-font-weight-700 g-font-size-20 mb-4">
					<?= get_lang('cant-found-the-solution-you-are-looking-for?') ?>
				</div>
				<a class="btn btn-info btn-block g-color-white g-font-size-13 g-font-weight-700 g-rounded-50 g-brd-2 g-py-10 text-uppercase" href="<?= site_url('service') ?>"><?= strtoupper(get_lang('all-services')) ?></a>
				<a class="btn btn-outline-info btn-block g-font-size-13 g-font-weight-700 g-rounded-50 g-brd-2 g-py-10 text-uppercase" href="<?= site_url('contact') ?>"><?= strtoupper(get_lang('talk-to-our-expert')) ?></a>
			</div>
		</div>
	</div>
	<?php
				...
				<div class="row">
					...
					foreach ($target->list as $service) {
						if ($service->service_id != $service_id && $i < $limit) {
							?>
							<div class="col-4">
								<div class="service-list-<?=$target->service_target_id?> g-mb-5 g-bg-white <?= $this->agent->is_mobile() ? 'g-mt-10' : 'g-mt-25' ?> box-shadow-down">
									<div style="height: 200px;background: url(<?= get_image(DIR_SERVICE . $service->service_slug . '/bg/' . $service->service_thumbnail_image) ?>);background-size: cover;"></div>
					...
					?>
					<div class="col-4">
						<div class="service-all-<?=$target->service_target_id?> g-mb-5 g-bg-white <?= $this->agent->is_mobile() ? 'g-mt-10' : 'g-mt-25' ?> box-shadow-down">
							<div class="text-center bg-white g-mx-20 g-px-30 g-py-20" style=" height:453px">
								<div class="g-mt-100">
									<div class="g-font-weight-700 g-font-size-20 mb-4">
										<?= get_lang('cant-found-the-solution-you-are-looking-for?') ?>
									</div>
									<a class="btn btn-info btn-block g-color-white g-font-size-13 g-font-weight-700 g-brd-2 g-rounded-50 g-py-10 text-uppercase" href="<?= site_url('service') ?>"><?= strtoupper(get_lang('all-services')) ?></a>
									<a class="btn btn-outline-info btn-block g-font-size-13 g-font-weight-700 g-brd-2 g-rounded-50 g-py-10 text-uppercase" href="<?= site_url('contact') ?>"><?= strtoupper(get_lang('talk-to-our-expert')) ?></a>
								</div>
							</div>
						</div>
					</div>
...
<script>
    $(window).ready(function () {
        ...
        var detail = $(".portfolio-list-<?=$target->service_target_id?>");
        var tempHeight = 0;
        ...
        $(".portfolio-all-<?=$target->service_target_id?>").height(tempHeight);
    });
