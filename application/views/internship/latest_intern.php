	...
	<div class="container">
		<div class="g-mb-50">
			<?php if (!$this->agent->is_mobile()) : ?>
				<a class="btn btn-default float-right g-rounded-50 g-py-10 g-px-80 g-font-size-18 text-uppercase" href="<?= site_url('internship/intern-list') ?>" style="font-weight:bold;color:#17A2B8"><?= $copy->latest_job->CTA ?><i class="ml-2 fa fa-angle-right"></i></a>
			<?php endif; ?>
			...
			<hr class="g-width-70 g-mt-20 g-mb-10 <?= $this->agent->is_mobile() ? '' : 'g-ml-0' ?> g-brd-2 w4c-brd-blue">
		</div>
		<div class="row justify-content-center align-height-list">
			...
			foreach ($jobs as $index => $item) :
				// if ($i <= $show && $item->status == 1 && !empty($item->talentaUrl)) :
					if ($i <= $show && $item->status == 1 && $item->is_new == 1) : 
			?>
					<div class="col-md-4 col-12  g-line-height-1_2 <?= $this->agent->is_mobile() ? 'g-px-10 g-py-5' : 'g-pa-5' ?>">
						<div class="g-bg-white <?= $this->agent->is_mobile() ? 'g-px-25 g-py-20' : 'g-pa-30 align-height-item' ?>" style="box-shadow:1px 6px 20px 2px rgba(0, 0, 0, 0.1);position:relative;">
							...
							<div class="g-font-size-<?= $this->agent->is_mobile() ? '12' : '16' ?> text-uppercase g-mb-20"><?= $internship_category->{$item->category}->name ?></div>
							<div class="w4c-color-blue d-block <?= $this->agent->is_mobile() ? '12' : '16' ?> text-uppercase g-pb-30 row no-gutters">
							...
							<div class="g-pos-abs" style="right:0px;bottom:30px;<?= $this->agent->is_mobile() ? 'bottom:20px;right:25px' : 'bottom:30px;right:30px' ?>">
								<a style="font-weight:bold" href="<?= site_url('internship/intern-list/' . $index) ?>" class="w4c-color-blue g-font-size-<?= $this->agent->is_mobile() ? '14' : '18' ?> text-uppercase"><?= $copy->latest_job->SeeDetail ?></a>
							</div>
						</div>
					</div>
			<?php $i++;
		...
		<?php if ($this->agent->is_mobile()) : ?>
			<div class="text-center g-mt-50 g-mb-80">
				<a class="btn btn-info btn-block g-rounded-50 g-py-10 g-px-80 g-font-size-14 text-uppercase" href="<?= site_url('internship/intern-list') ?>"><?= $copy->latest_job->CTA ?></a>
			</div>
		<?php endif; ?>
	</div>
</section>
