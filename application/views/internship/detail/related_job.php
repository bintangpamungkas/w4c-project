  ...
  <div class="container">
    <div class="g-mb-50">
      <?php if (!$this->agent->is_mobile()) : ?>
        <a class="btn btn-info float-right g-rounded-50 g-py-10 g-px-80 g-font-size-18 text-uppercase" href="<?= site_url('internship/intern-list') ?>"><?= $copy->SeeMore ?></a>
      <?php endif; ?>
    ...
    <div class="container">
          ...
          ?>
            <div class="col-md-4 col-12 g-line-height-1_2 <?= $this->agent->is_mobile() ? 'g-px-10 g-py-5' : 'g-pa-5' ?>">
              <div class="g-bg-white <?= $this->agent->is_mobile() ? 'g-px-25 g-py-20' : 'g-pa-30 align-height-item' ?>" style="box-shadow:1px 6px 20px 2px rgba(0, 0, 0, 0.1);position:relative;">
                  ...
                  <div class="col-md-12 timeline pt-2">
                      <div class="d-inline">
                  ...
                  <div class="g-pos-abs" style="right:0px;bottom:30px;<?= $this->agent->is_mobile() ? 'bottom:20px;right:25px' : 'bottom:30px;right:30px' ?>">
                    <a href="<?= site_url('internship/intern-list/' . $itemi) ?>" class="w4c-color-blue g-font-size-<?= $this->agent->is_mobile() ? '14' : '18' ?> text-uppercase"><?= $copy->SeeDetail ?></a>
                  </div>
              </div>
            </div>
        <?php $i++;
      ...
      <?php if ($this->agent->is_mobile()) : ?>
        <div class="text-center g-mt-50 g-mb-80">
          <a class="btn btn-info btn-block g-rounded-50 g-py-10 g-px-80 g-font-size-14 text-uppercase" href="<?= site_url('internship/intern-list') ?>"><?= $copy->SeeMore ?></a>
        </div>
      <?php endif; ?>
